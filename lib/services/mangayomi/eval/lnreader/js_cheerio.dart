    ...
    runtime.onMessage('element_call', (dynamic args) {
      ...
      switch (method) {
        ...
        case 'parent':
          ...
          break;
        case 'find':
          final selector = methodArgs[0]?.toString() ?? '';
          ...
          break;
        case 'first':
          final first = element.children.isNotEmpty
          ...
          break;
        case 'last':
          final last = element.children.isNotEmpty
        ...
        case 'prev':
          ...
          break;
        case 'append':
          final htmlToAppend = methodArgs[0]?.toString() ?? '';
          final newNodes = parse(htmlToAppend).body?.children ?? [];
          for (var node in newNodes) {
            element.append(node);
          }
          break;
        case 'prepend':
          final htmlToPrepend = methodArgs[0]?.toString() ?? '';
          final newNodes = parse(htmlToPrepend).body?.children ?? [];
...
class Element {
  ...
  parent() {
    const res = this._call("parent");
    return res ? new Element(res) : new Stub();
  }

  next() {
    const res = this._call("next");
    return res ? new Element(res) : new Stub();
  }

  prev() {
    const res = this._call("prev");
    return res ? new Element(res) : new Stub();
  }

  first() {
    const res = this._call("first");
    return res ? new Element(res) : new Stub();
  }

  last() {
    const res = this._call("last");
    return res ? new Element(res) : new Stub();
  }
}
