...
import 'package:zenbu/services/mangayomi/eval/model/m_provider.dart';
import 'package:zenbu/services/mangayomi/webview_server.dart';
import 'package:zenbu/services/mangayomi/utils/extensions/string_extensions.dart';
...
class MProviderBridged {
  ...
  void registerBridgedClasses(D4rt interpreter) {
    ...
    interpreter.registertopLevelFunction(
      'toVideo',
      (visitor, positionalArgs, namedArgs, _) => MBridge.toVideo(
        positionalArgs[0] as String,
        positionalArgs[1] as String,
        positionalArgs[2] as String,
        positionalArgs.length > 3 ? positionalArgs[3] as String? : null,
        positionalArgs.length > 4 ? positionalArgs[4] as List? : null,
        positionalArgs.length > 5 ? positionalArgs[5] as List? : null,
      ),
    );
    interpreter.registertopLevelFunction('evaluateJavascriptViaWebview', (
      visitor,
      positionalArgs,
      namedArgs,
      _,
      ...
      try {
        ...
        final headers = positionalArgs.length > 1
            ? (positionalArgs[1] as Map?)?.cast<String, String>() ??
                  <String, String>{}
            : <String, String>{};
        final scripts = positionalArgs.length > 2
            ? (positionalArgs[2] as List?)?.cast<String>() ?? <String>[]
            : <String>[];
        final time = positionalArgs.length > 3
            ? positionalArgs[3] as int? ?? 30
            : 30;
    ...
    interpreter.registertopLevelFunction('print', (
      ...
      if (kDebugMode) {
        debugPrint("[DART EVAL]: ${positionalArgs[0]}");
      }
...
class ExtPreferences {
  static final Map<String, Map<String, dynamic>> _prefs = {};
  static void init(int sourceId, Map<String, dynamic> values) {
    _prefs[sourceId.toString()] = values;
  }

  static dynamic get(int sourceId, String key) {
    return _prefs[sourceId.toString()]?[key];
  }
}

dynamic getPreferenceValue(int sourceId, String key) {
  final value = ExtPreferences.get(sourceId, key);
  if (value != null) return value;
  if (key == 'hoster_selection') {
    return <String>['voe', 'dood', 'filemoon', 'mp4upload'];
  }
  if (key.contains('selection') ||
      key.contains('multi') ||
      key.contains('list') ||
      key.contains('hosters')) {
    return <dynamic>[];
  }
  return '';
}
...
String getSourcePreferenceStringValue(
  ...
  return ExtPreferences.get(sourceId, key)?.toString() ?? defaultValue;
}
